...
export class TestRunner {
  ...
  renderAllQuestions(questions) {
    ...
    questions.forEach((qData, index) => {
      ...
      answers.forEach((answer, index) => {
        ...
        // 1. Parsujemy treść odpowiedzi z Markdown na HTML za pomocą biblioteki marked.
        //    Opcja `gfm: true` włącza GitHub Flavored Markdown dla lepszej kompatybilności.
        //    Opcja `breaks: true` sprawia, że pojedyncze entery tworzą <br>.
        const unsafeHTML = marked.parse(answer.content, { gfm: true, breaks: true });

        // 2. Zabezpieczamy wygenerowany HTML za pomocą DOMPurify, aby uniknąć XSS.
        //    To bardzo ważny krok, jeśli treść może pochodzić od użytkowników.
      ...
      questionElement.innerHTML = `
                ...
                <div class="quiz-card__content">
                    ...
                    ${
                      question.image_path
                        ? `<div class="quiz-card__image-container"><img src="/examly/public/images/questions/${escapeHTML(question.image_path)}" alt="Ilustracja do pytania" class="quiz-card__image"></div>`
                        : ''
                    }
                </div>
  ...
  showResults(score, correctCount, userAnswers) {
    ...
    document.getElementById('duration-value').textContent = `${String(durationMinutes).padStart(2, '0')}:${String(
      durationSeconds
    ).padStart(2, '0')}`;
    ...
    this.questionsData.forEach((qData) => {
      ...
      if (correctAnswer) {
        const correctLabel = questionCard
          .querySelector(`input[value="${correctAnswer.id}"]`)
          ?.closest('.quiz-card__answer');
        if (selectedAnswerId !== null) {
          const selectedLabel = questionCard
            .querySelector(`input[value="${selectedAnswerId}"]`)
            ?.closest('.quiz-card__answer');
